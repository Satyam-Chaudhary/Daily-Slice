...
export default function SearchResults() {
  ...
  const newsResults = isNewsFetching ? (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {news.articles.map(article => <ContentCard key={article.url} article={article} layoutType="grid"  />)}
    </div>
  ) : (
  ...
  const movieResults = isMoviesFetching ? (
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
      {movies.results.map(movie => <MovieCard key={movie.id} movie={movie} layoutType="grid" />)}
    </div>
  ) : (
  ...
  const socialResults = isSocialLoading ? (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {filteredSocialPosts.map(post => <SocialPostCard key={post.id} post={post} layoutType="grid" />)}
    </div>
  ) : (
